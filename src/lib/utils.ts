...
export const convertToMarkdown = (errors: ParsedError[], additionalNotes: string): string => {
    const errorMarkdown = errors.map(({ summary, fix }) => `### ${summary}\n- **Fix:** ${fix}`).join('\n\n');
    const notes = additionalNotes || "Implement these improvements and fixes systematically while maintaining existing functionality.";
    return `# Error Report\n\n## Additional Notes\n${notes}\n\n${errorMarkdown}`;
}
